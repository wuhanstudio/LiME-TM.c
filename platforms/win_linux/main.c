...
int main(int argc, char* argv[]) {
    ...
    // int rows, cols;
    // uint32_t train_img_count = mnist_image_info(MNIST_TRAIN_IMG_PATH, &rows, &cols);
    // LOGI(TAG, "MNIST training set: %d images of size %dx%d", train_img_count, rows, cols);

    // uint32_t train_label_count = mnist_label_info(MNIST_TRAIN_LABEL_PATH);
    ...
    // // Get test set info
    // uint32_t test_img_count = mnist_image_info(MNIST_TEST_IMG_PATH, &rows, &cols);
    // LOGI(TAG, "MNIST test set: %d images of size %dx%d", test_img_count, rows, cols);

    // uint32_t test_label_count = mnist_label_info(MNIST_TEST_LABEL_PATH);
    ...
    // // Load Tsetlin model from file
    // size_t size = 0;
    // uint8_t* data = tsetlin_read_file(DISK_MOUNT_PT"/tsetlin_model_8_bit.cpb", &size);
    // if (!data) {
    //     LOGE(TAG, "Failed to read file\n");
    //     return -1;
    // }
    ...
    // printf("Predicted class: %u with %d votes\n", predicted_class, votes[predicted_class]);
    // for (size_t i = 0; i < model->n_class; i++)
    // {
    //     printf("Class %lu: %d votes\n", i, votes[i]);
    // }
    ...
    //     uint8_t* img = mnist_load_next_image(f_test_imgs, i, rows, cols);
    //     if (!img) {
    //         printf("Failed to load test image %u\n", i);
    //         continue;
    //     }

    //     int8_t label = mnist_load_next_label(f_test_labels, i);
    //     if (label < 0) {
    //         printf("Failed to load test label %u\n", i);
    //         free(img);
    //         continue;
    //     }
    ...
    //     if ((i + 1) % 1000 == 0) {
    //         char message[64];
    //         snprintf(message, sizeof(message), "Testing %u/%u", i + 1, test_img_count);
    //         print_progress(message, (i + 1) * 100 / test_img_count);
    //     }
    // }
    // printf("\n");
    ...
    // printf("Accuracy on test set (%u): %.2f%%\n", test_img_count, (double)correct / test_img_count * 100);
    ...
    //         uint8_t* X_img = mnist_load_next_image(f_train_imgs, j, rows, cols);
    //         if (!X_img) {
    //             printf("Failed to load train image %u\n", j);
    //             continue;
    //         }

    //         int8_t y_target = mnist_load_next_label(f_train_labels, j);
    //         if (y_target < 0) {
    //             printf("Failed to load train label %u\n", j);
    //             continue;
    //         }
    ...
    //         if ((j + 1) % 1000 == 0) {
    //             char message[64];
    //             snprintf(message, sizeof(message), "Epoch %lu: Processed %u/%u", i + 1, j + 1, train_img_count);
    //             print_progress(message, (j + 1) * 100 / train_img_count);
    //         }
    //     }
    ...
    //         if ((j + 1) % 1000 == 0) {
    //             char message[32];
    //             snprintf(message, sizeof(message), "Testing %u/%u", j + 1, test_img_count);
    //             print_progress(message, (j + 1) * 100 / test_img_count);
    //         }
    //     }
    //     printf("\n");
    //     printf("Testing Accuracy after epoch %lu: %.2f%%\n", i + 1, (double)correct / test_img_count * 100);
    // }
