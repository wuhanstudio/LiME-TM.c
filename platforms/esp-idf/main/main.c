...
int lime_tm_mnist_test_all(Tsetlin* model, FILE* f_test_imgs, FILE* f_test_labels, int test_img_count, int rows, int cols, int32_t* votes)
{
    ...
    {
        uint32_t start_utility = xTaskGetTickCount();
        ...
        if ((i + 1) % 1000 == 0) {
            char message[32];
            snprintf(message, sizeof(message), "Testing %ld/%d", i + 1, test_img_count);
...
int lime_tm_mnist_main() {
    ...
    for (size_t i = 0; i < N_EPOCHS; i++)
    {
        for (uint32_t j = 0; j < train_img_count; j++)
        {
            ...
            if ((j + 1) % 1000 == 0) {
                char message[32];
                snprintf(message, sizeof(message), "Epoch %u: Processed %ld/%ld", i + 1, j + 1, train_img_count);
